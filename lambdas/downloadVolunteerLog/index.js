...
const GOOGLE_CREDENTIALS = JSON.parse(process.env.HNJ_VOLUNTEERS_JWT),
  ...
  downloadSignInLog = require('./downloadSignInLog').downloadSignInLog;
...
module.exports.downloadSignInLog = function (event, context, callback) {
  getAuthenticationContext(GOOGLE_CREDENTIALS, SCOPES)
    .then(authenticationContext => {
      return downloadVolunteers(authenticationContext, "1nt0w3FpVl2RPbloI7H_tGWIaGF38PfXdcm7UnrpWXJM", "Signup!A2:E");
    })
    .then(volunteers => {
      let response = {
        statusCode: 200,
